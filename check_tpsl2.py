...
for p in positions.get('data', []):
    if float(p.get('positions', 0)) != 0:
        ...
        print(f"{p['instId']}: side={p['positionSide']} size={p['positions']} entry={p['averagePrice']} TP={tp} SL={sl}")
...
for p in positions.get('data', []):
    if float(p.get('positions', 0)) != 0:
        ...
        tp_orders = [o for o in orders.get('data', []) if o.get('ordType') == 'take_profit']
        sl_orders = [o for o in orders.get('data', []) if o.get('ordType') == 'stop_loss']
