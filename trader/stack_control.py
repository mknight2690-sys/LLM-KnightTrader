...
def _is_trader_process_cmd(cmd: str) -> bool:
    ...
    if "trader\\agent.py" in compact or "trader/agent.py" in compact:
        return True
    # Some launchers may inline-call the trader entrypoint.
    if "from trader.agent import main" in compact and "main()" in compact:
        return True
    return False
...
def trader_lock_owner() -> int | None:
    ...
    Kept local to avoid hard dependency on health.py lock internals.
    """
...
def restart_module(module: str) -> dict[str, Any]:
    """
    Restart a specific stack component after a patch.

    For now, we keep this conservative: trader can restart in-place; everything
    else falls back to a cold full-stack restart (more reliable on novel breakage).
    """
    m = str(module or "").strip().lower()
    if m in {"trader.agent", "trader", "traderagent"}:
        # Trader restart is isolated; we don't kill dashboard.
        result = start_single_trader()
        return {"ok": bool(result.get("ok")), "module": module, "result": result}

    if m in {"dashboard.server", "dashboard", "dashboardserver", "monitor.agent", "monitor"}:
        result = restart_traders()
        return {"ok": bool(result.get("ok")), "module": module, "result": result}

    # Unknown module => safe fallback.
    result = restart_traders()
    return {"ok": bool(result.get("ok")), "module": module, "result": result}
